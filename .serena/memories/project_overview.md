...
This project is a starter template for building decentralized applications (dApps) on the Midnight Network. It features a React-based frontend, a smart contract, and a command-line interface (CLI) for interacting with the contract. The project is structured as a monorepo managed by pnpm workspaces and Turbo.
...
- `pnpm run build`: Builds all the workspaces.
- `pnpm run test`: Runs tests for all the workspaces.
- `pnpm run lint`: Lints all the workspaces.
- `pnpm run start-app-testnet`: Starts the application in Testnet mode. This runs the proof server and the frontend development server concurrently.
- `pnpm run start-app-undeployed`: Starts the application in local (undeployed) mode. This runs the frontend development server.
- `pnpm run dev:undeployed-instances`: Prepares the local development environment.
